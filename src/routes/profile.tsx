...
export default function Profile() {
    ...
    const onAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (files && files.length === 1) { // 선택된 파일이 있고 파일이 1개인 경우에만 실행됩니다. 
    ...
    const fetchTweets = async () => { //fetchTweets 함수는 비동기 함수로, 사용자의 트윗을 가져오는 데 사용
    ...
    const handleDelete = async (tweetId: string) => { //handleDelete 함수는 트윗의 ID를 매개변수로 받습니다.
        ...
        } catch (error) { // try-catch 블록은 삭제 작업이 실패할 경우를 처리합니다. 만약 오류가 발생하면, 콘솔에 오류 메시지를 기록
